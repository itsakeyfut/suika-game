...
/// `min_fruit_index` (default [`SHAKE_MIN_INDEX`]), increments the
...
/// 1. Decays `trauma` by `decay` × `delta_secs` (from config or [`SHAKE_DECAY`])
/// 2. Computes `shake_amount = trauma²`
/// 3. Applies a random X/Y offset to the camera `Transform`, scaled by
///    `shake_amount × max_offset` (from config or [`SHAKE_MAX_OFFSET`])
...
mod tests {
    ...
    fn test_small_fruits_do_not_trigger_shake() {
        // Fruits below DEFAULT_SHAKE_MIN_INDEX should produce zero intensity
        for index in 0..DEFAULT_SHAKE_MIN_INDEX {
            assert!(
                index < DEFAULT_SHAKE_MIN_INDEX,
                "Index {} should be below min threshold {}",
                index,
                DEFAULT_SHAKE_MIN_INDEX
            );
        }
    }
    ...
    fn test_large_fruit_intensity_scales_up() {
        // Watermelon is index 10, well above DEFAULT_SHAKE_MIN_INDEX (5)
